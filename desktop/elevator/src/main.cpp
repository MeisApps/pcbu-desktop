  ...
  try {
    switch(cmd.type) {
      ...
      case ElevatorCommandType::READ_BYTES: {
        ...
        if(data.empty()) {
          send_response(sendQueue, ElevatorCommandResponse(0, 0));
          break;
        }
        auto shmHandle = IPCHelper::WriteShmBytes(sharedMem, data);
        if(!shmHandle.has_value()) {
          send_response(sendQueue, ElevatorCommandResponse(true, "Failed to write shared memory."));
          break;
        }
        send_response(sendQueue, ElevatorCommandResponse(shmHandle.value(), data.size()));
        break;
      }
...
int main(int argc, char *argv[]) {
  ...
  LoggingSystem::Init("elevator", false, true, true);
  ...
  do {
    ...
    spdlog::debug("[Elevator] Running commmand {}...", (int)cmd.type);
