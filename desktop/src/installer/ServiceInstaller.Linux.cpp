...
#define PAM_CONFIG_ENTRY_SDDM "auth [success=1 new_authtok_reqd=1 default=ignore] pam_unix.so try_first_pass likeauth nullok\nauth sufficient pam_pcbiounlock.so"
...
std::vector<ServiceSetting> ServiceInstaller::GetSettings() {
    auto isLoginEnabled = PAMHelper::HasConfigEntry("gdm-password", PAM_CONFIG_ENTRY)
            || PAMHelper::HasConfigEntry("sddm", PAM_CONFIG_ENTRY_SDDM)
            || PAMHelper::HasConfigEntry("kde", PAM_CONFIG_ENTRY)
            || PAMHelper::HasConfigEntry("lightdm", PAM_CONFIG_ENTRY)
            || PAMHelper::HasConfigEntry("cinnamon-screensaver", PAM_CONFIG_ENTRY);
    auto hasLoginManager = IsProgramInstalled(GDM_NAME)
            || IsProgramInstalled(SDDM_NAME)
