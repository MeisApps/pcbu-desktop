...
QHash<int, QByteArray> NetworkListModel::roleNames() const {
    return {{InterfaceNameRole, "ifName"},
            {IPAddressRole, "ipAddress"},
            {MACAddressRole, "macAddress"},
            {IfWithIPRole, "ifWithIp"}};
}
...
QVariant NetworkListModel::data(const QModelIndex & index, int role) const {
    ...
    if (role == InterfaceNameRole) return item.ifName;
    if (role == IPAddressRole) return item.ipAddress;
    if (role == MACAddressRole) return item.macAddress;
    if (role == IfWithIPRole) return QString("%1 (%2)").arg(item.ipAddress, item.ifName);
    return {};
}
