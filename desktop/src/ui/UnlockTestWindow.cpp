...
void UnlockTestWindow::StartUnlock(QObject *window, const QString &deviceId) {
  ...
  m_UnlockHandler = std::make_unique<UnlockHandler>(
      [window](const std::string &s) { QMetaObject::invokeMethod(window, "setUnlockMessage", Q_ARG(QVariant, QString::fromUtf8(s))); });
  ...
  m_UnlockThread = std::thread([this, window, device]() {
    ...
    m_UnlockHandler->GetResult(device.value().userName, I18n::Get("unlock_test"), {device.value().id}, &m_IsRunning);
