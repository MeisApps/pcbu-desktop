...
std::vector<NetworkInterface> NetworkHelper::GetLocalNetInterfaces() {
  ...
  for(PIP_ADAPTER_ADDRESSES adapter = adapterAddresses; adapter; adapter = adapter->Next) {
    ...
    char macBuffer[18]{};
    snprintf(macBuffer, sizeof(macBuffer), "%02X:%02X:%02X:%02X:%02X:%02X", adapter->PhysicalAddress[0], adapter->PhysicalAddress[1],
  ...
  for(auto ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    if(family == AF_INET || family == AF_PACKET) {
      ...
      auto res = getnameinfo(ifa->ifa_addr, (family == AF_INET) ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6), addr, NI_MAXHOST, nullptr,
                             0, NI_NUMERICHOST);
...
bool NetworkHelper::HasLANConnection() {
  ...
  if(FAILED(hr)) {
    ...
    if(FAILED(hr)) {
      ...
      hasLAN = nlmConnectivity & NLM_CONNECTIVITY_IPV4_SUBNET || nlmConnectivity & NLM_CONNECTIVITY_IPV4_LOCALNETWORK ||
