...
constexpr int QUEUE_RW_TIMEOUT = 500;
...
std::optional<std::string> IPCHelper::ReadString(boost::interprocess::message_queue &mq, const std::function<bool()> &isRunning, uint32_t timeoutMs) {
  try {
    ...
    while(boost::posix_time::microsec_clock::universal_time() < deadline) {
      auto readTimeout = boost::posix_time::microsec_clock::universal_time() + boost::posix_time::milliseconds(QUEUE_RW_TIMEOUT);
...
bool IPCHelper::WriteString(boost::interprocess::message_queue &mq, const std::string &str, const std::function<bool()> &isRunning,
  ...
  try {
    ...
    auto deadline = boost::posix_time::microsec_clock::universal_time() + boost::posix_time::milliseconds(timeoutMs);
    while(boost::posix_time::microsec_clock::universal_time() < deadline) {
      auto writeTimeout = boost::posix_time::microsec_clock::universal_time() + boost::posix_time::milliseconds(QUEUE_RW_TIMEOUT);
...
std::vector<uint8_t> IPCHelper::ReadShmBytes(boost::interprocess::managed_shared_memory &sharedMem,
  ...
  try {
    if(length == 0 || handle == 0)
      return {};
    auto size = sharedMem.get_size();
    auto offset = static_cast<size_t>(handle);
    if(offset >= size || length > size - offset)
      return {};
    auto shmPtr = static_cast<uint8_t *>(sharedMem.get_address_from_handle(handle));
...
std::optional<boost::interprocess::managed_shared_memory::handle_t> IPCHelper::WriteShmBytes(boost::interprocess::managed_shared_memory &sharedMem,
  ...
  try {
    ...
    auto handle = sharedMem.get_handle_from_address(shmPtr);
    if(handle == 0) {
      sharedMem.deallocate(shmPtr);
      return {};
    }
